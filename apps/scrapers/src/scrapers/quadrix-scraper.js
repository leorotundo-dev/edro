...
class QuadrixScraper extends BaseScraper {
  constructor() {
    super('Quadrix', 'https://quadrix.selecao.net.br');
  }

  async scrape() {
    console.log(`\n[${this.name}] Starting concursos list...\n`);

    try {
      const listPages = [
        `${this.baseUrl}/index/abertos/`,
        `${this.baseUrl}/index/1/`,
      ];
      ...
      for (const listUrl of listPages) {
        try {
          ...
          $('a[href^="/informacoes/"]').each((_, el) => {
            ...
            const titulo =
              $(el).text().trim() ||
              $(el).closest('li, tr, .item').find('h3, h4').first().text().trim();
            if (url) {
              concursos.set(url, { url, titulo: titulo || url });
            }
          });
        } catch (error) {
          console.error(`[${this.name}] List error (${listUrl}): ${error.message}`);
        }
      }

      const items = Array.from(concursos.values());
      console.log(`[${this.name}] Found ${items.length} concursos`);

      const configuredLimit = parseInt(
        process.env.QUADRIX_LIMIT || process.env.SCRAPER_LIMIT || '10',
        10
      );
      const limit = Math.min(Number.isFinite(configuredLimit) ? configuredLimit : 10, items.length);
      console.log(`[${this.name}] Processing ${limit} concursos...\n`);
      ...
      for (let i = 0; i < limit; i++) {
        ...
        try {
          ...
          const editalLink = pdfLinks.find((item) => {
            ...
            return item.label.includes('edital') || lower.includes('edital');
          });
