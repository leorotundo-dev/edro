...
class InstitutoMaisScraper extends BaseScraper {
  constructor() {
    super('Instituto Mais', 'https://www.institutomais.org.br');
  }
  ...
  async scrape() {
    ...
    try {
      const listUrl = `${this.baseUrl}/Concursos/ConcursosAbertos`;
      const html = await fetchHtml(listUrl);
      const $ = cheerio.load(html);
      const concursos = new Map();

      $('a[href*="/Concursos/Detalhe/"]').each((_, el) => {
        ...
        const titulo =
          $(el).text().trim() ||
          $(el).closest('li, tr, .item, .box').find('h3, h4').first().text().trim();
      ...
      const configuredLimit = parseInt(
        process.env.INSTITUTOMAIS_LIMIT || process.env.SCRAPER_LIMIT || '10',
        10
      );
      ...
      for (let i = 0; i < limit; i++) {
        ...
        try {
          ...
          await this.saveEdital({
            fonte: 'Instituto Mais',
            titulo,
            url: editalUrl,
            noticia_url: concurso.url,
            edital_url: editalUrl,
            pdf_links: pdfLinks,
            site_fonte: 'Instituto Mais',
          });
...
module.exports = InstitutoMaisScraper;
