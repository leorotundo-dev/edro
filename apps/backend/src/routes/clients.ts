...
Omita campos que não encontrou informação confiável. Para segment_primary, sempre escolha a opção mais próxima da lista acima.`;
  ...
  // GET /jarvis/feed — lightweight proactive feed for Home command center
  ...
  app.get('/jarvis/feed', {
    ...
    const tenantId = request.user.tenant_id;
    try {
      const [briefingPendingRes, alertsRes, autoBriefingsRes, proposalsRes, opportunitiesRes] = await Promise.all([
        // Jobs awaiting briefing
        query(
          `SELECT j.id, j.title, j.job_type, j.status, j.created_at,
           ...
           WHERE j.tenant_id = $1
             AND j.automation_status = 'briefing_pending'
             AND j.status NOT IN ('done', 'archived', 'cancelled')
           ORDER BY j.created_at DESC
           LIMIT 10`,
          [tenantId],
        ).catch(() => ({ rows: [] as any[] })),
        // Urgent/high account manager alerts
        query(
          `SELECT ama.id, ama.client_id, c.name AS client_name,
                  ama.alert_type, ama.priority, ama.title, ama.body,
                  ama.recommended_action, ama.computed_at
           FROM account_manager_alerts ama
           JOIN clients c ON c.id = ama.client_id
           WHERE ama.tenant_id = $1
             AND ama.status = 'active'
             AND ama.priority IN ('urgent', 'high')
           ORDER BY
             CASE ama.priority WHEN 'urgent' THEN 1 ELSE 2 END,
             ama.computed_at DESC
           LIMIT 5`,
          [tenantId],
        ).catch(() => ({ rows: [] as any[] })),
        // Auto-briefings last 48h not yet approved
        query(
          `SELECT b.id, b.title, b.source, b.status, b.created_at,
                  c.id AS client_id, c.name AS client_name,
                  (b.payload->>'drop_pct')::int AS drop_pct,
                  (b.payload->>'opportunity_confidence')::int AS opportunity_confidence
           FROM edro_briefings b
           LEFT JOIN clients c ON c.id::text = b.main_client_id::text
           WHERE (b.tenant_id IS NULL OR EXISTS (
             SELECT 1 FROM clients c2 WHERE c2.id::text = b.main_client_id::text AND c2.tenant_id = $1
           ))
             AND b.source IN ('auto_opportunity', 'fatigue_substitution')
             AND b.status NOT IN ('approved', 'published')
             AND b.created_at >= NOW() - INTERVAL '48 hours'
           ORDER BY b.created_at DESC
           LIMIT 5`,
          [tenantId],
        ).catch(() => ({ rows: [] as any[] })),
        // Pending meeting proposals (Jarvis extracted actions awaiting approval)
        query(
          `SELECT ma.id, ma.meeting_id, ma.action_type, ma.title, ma.description,
                  ma.suggested_status, ma.created_at,
        ...
        // High-confidence AI opportunities
        query(
          `SELECT o.id, o.title, o.description, o.priority, o.confidence,
                  o.suggested_action, o.created_at,
                  c.id AS client_id, c.name AS client_name
           FROM ai_opportunities o
           JOIN edro_clients ec ON ec.id = o.client_id
           JOIN clients c ON LOWER(c.name) = LOWER(ec.name)
      ...
      return reply.send({
        briefing_pending: briefingPendingRes.rows,
        alerts:           alertsRes.rows,
        auto_briefings:   autoBriefingsRes.rows,
        proposals:        proposalsRes.rows,
        opportunities:    opportunitiesRes.rows,
        total_actions:
          briefingPendingRes.rows.length +
          alertsRes.rows.length +
          autoBriefingsRes.rows.length +
          proposalsRes.rows.length +
          opportunitiesRes.rows.length,
      });
    } catch (err: any) {
      return reply.status(500).send({ error: err.message });
    }
