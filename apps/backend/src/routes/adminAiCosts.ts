...
async function ensurePlatformCostsTable() {
  await query(`
    CREATE TABLE IF NOT EXISTS edro_platform_costs_tenant (
  ...
  `);
  await query(`
    CREATE UNIQUE INDEX IF NOT EXISTS idx_edro_platform_costs_tenant_platform
    ON edro_platform_costs_tenant (tenant_id, platform)
  `);
}

async function loadPlatformCosts(tenantId: string) {
  const tenantRows = await query<{ platform: string; monthly_cost_usd: string | number }>(
    `SELECT platform, monthly_cost_usd
     FROM edro_platform_costs_tenant
     WHERE tenant_id = $1`,
    [tenantId],
  );
  ...
  for (const row of tenantRows.rows) {
    fixedMap[row.platform] = Number(row.monthly_cost_usd);
  }

  // Backward compatibility for old shared table: use only as read fallback.
  if (Object.keys(fixedMap).length === 0) {
    const legacyRows = await query<{ platform: string; monthly_cost_usd: string | number }>(
      `SELECT platform, monthly_cost_usd
       FROM edro_platform_costs`,
    ).catch(() => ({ rows: [] as Array<{ platform: string; monthly_cost_usd: string | number }> }));
...
export default async function adminAiCostsRoutes(app: FastifyInstance) {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const [realCosts, recallData, fixedMap] = await Promise.all([
        query(
  ...
  app.patch(
    ...
    async (request, reply) => {
      ...
      await query(
        `INSERT INTO edro_platform_costs_tenant (tenant_id, platform, monthly_cost_usd)
         VALUES ($1, $2, $3)
         ON CONFLICT (tenant_id, platform)
         DO UPDATE SET monthly_cost_usd = EXCLUDED.monthly_cost_usd, updated_at = now()`,
        [tenantId, platform, monthly_cost_usd],
      );
