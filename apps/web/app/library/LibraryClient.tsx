  ...
  const content = (
      <div className="page-content">
        ...
        <div className="flex flex-col gap-10">
          <div>
            <h1 className="font-display text-5xl text-ink mb-2">Global Reference Library Hub</h1>
            <p className="text-muted">
              Centralized repository for operational assets, brand guidelines, and reference materials.
            </p>
          </div>

          <section className="bg-surface-light border-2 border-dashed border-border rounded-2xl p-10 text-center transition-colors hover:border-primary/50 group">
          ...
          <section className="space-y-4">
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-surface-light border border-border p-4 rounded-xl shadow-sm">
              <div className="flex items-center gap-4 w-full md:w-auto">
                <div className="relative flex-1 md:w-64">
                  ...
                  <input
                    className="w-full pl-10 pr-4 py-2 bg-paper border-none rounded-lg text-sm focus:ring-1 focus:ring-primary"
                ...
                <select
                  className="bg-paper border-none rounded-lg text-sm py-2 px-4 pr-10 focus:ring-1 focus:ring-primary"
            ...
            <div className="bg-surface-light border border-border rounded-xl overflow-hidden shadow-sm">
                ...
                <tbody>
                  {items.map((item) => (
                    <tr key={item.id} className="border-b border-slate-100">
                      ...
                      <td className="px-6 py-4 text-xs text-muted uppercase tracking-widest">
                        {item.category || 'General'}
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-3 py-1 text-[10px] font-bold uppercase tracking-widest rounded-full bg-card-strong text-muted">
                          {item.status || 'ready'}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-xs text-muted uppercase tracking-widest">
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex items-center justify-end gap-2 row-action">
                          <button
                            className="px-3 py-1.5 text-xs font-semibold border border-border rounded-lg hover:text-primary"
                            type="button"
    ...
    <AppShell
      ...
      topbarLeft={
        <nav className="flex items-center space-x-2 text-sm text-slate-400">
          ...
          <span className="text-ink font-medium">Global Reference Library</span>
        </nav>
      }
    >
      {content}
