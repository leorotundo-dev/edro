...
type ClientAnalyticsCoreProps = {
  clientId?: string;
  forcedTab?: number;
  compact?: boolean;
};

export default function ClientAnalyticsCore({
  clientId: clientIdProp,
  forcedTab,
  compact = false,
}: ClientAnalyticsCoreProps) {
  const params = useParams();
  const routeClientId = params?.id as string | undefined;
  const clientId = clientIdProp || routeClientId || '';
  const [tab, setTab] = useState(forcedTab ?? 0);
  ...
  const activeTab = typeof forcedTab === 'number' ? forcedTab : tab;
  const showTabs = !compact && typeof forcedTab !== 'number';
