...
function EditTaskModal({ show, onHide, editedTask, onSave }) {
  ...
  const handleLabelChange = (newLabels) => {
    setTempEditedTask((prev) => ({ ...prev, labels: newLabels }));
  };
  ...
  const handleUserSelect = (user) => {
    const exists = assignedTo.some((u) => u.name === user.name);
    if (exists) {
      setAssignedTo(assignedTo.filter((u) => u.name !== user.name));
    } else {
  ...
  return (
    ...
    >
      <Box padding={3} display="flex" gap={3} flexDirection="column">
        ...
        {tab === 0 && (
          <Box>
            {tempEditedTask?.comments?.map((c, idx) => (
